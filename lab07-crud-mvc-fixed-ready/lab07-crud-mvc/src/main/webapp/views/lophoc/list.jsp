<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="jakarta.tags.core" %>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Quản lý lớp học</title>
    <link rel="stylesheet"
          href="${pageContext.request.contextPath}/css/style.css">
</head>

<body>

<div class="header">
    <div class="logo">🏫 LAB 07 <span>CRUD MVC</span></div>
    <div>Xin chào, <strong>${sessionScope.username}</strong></div>
</div>

<div class="navbar">
    <a href="${pageContext.request.contextPath}/admin">🏠 Trang chủ</a>
    <a href="${pageContext.request.contextPath}/sinhvien">🎓 Sinh viên</a>
    <a href="${pageContext.request.contextPath}/sach">📚 Sách</a>
    <a href="${pageContext.request.contextPath}/sanpham">🛍 Sản phẩm</a>
    <a href="${pageContext.request.contextPath}/lophoc">🏫 Lớp học</a>
    <a href="${pageContext.request.contextPath}/diem">📝 Điểm</a>
    <a href="${pageContext.request.contextPath}/giohang">🛒 Giỏ hàng</a>
    <a href="${pageContext.request.contextPath}/logout">🚪 Đăng xuất</a>
</div>

<div class="container">

    <h1 class="page-title">🏫 Quản lý lớp học</h1>

    <p class="page-description">
        Quản lý thông tin các lớp học.
    </p>

    <div class="card">

        <form method="get"
              action="${pageContext.request.contextPath}/lophoc"
              class="search-box">

            <input type="text"
                   name="keyword"
                   value="${keyword}"
                   placeholder="Nhập mã hoặc tên lớp...">

            <button class="btn btn-primary"
                    type="submit">
                🔍 Tìm kiếm
            </button>

            <a class="btn btn-success"
               href="${pageContext.request.contextPath}/lophoc?action=form">
                + Thêm lớp
            </a>

        </form>

        <div class="table-wrapper">

            <table class="table">

                <thead>
                <tr>
                    <th>ID</th>
                    <th>Mã lớp</th>
                    <th>Tên lớp</th>
                    <th>Cố vấn</th>
                    <th>Số lượng SV</th>
                    <th>Thao tác</th>
                </tr>
                </thead>

                <tbody>

                <c:choose>

                    <c:when test="${empty danhSach}">
                        <tr>
                            <td colspan="6"
                                style="text-align:center">
                                Không có dữ liệu.
                            </td>
                        </tr>
                    </c:when>

                    <c:otherwise>

                        <c:forEach var="lopHoc"
                                   items="${danhSach}">

                            <tr>

                                <td>${lopHoc.id}</td>

                                <td>${lopHoc.maLop}</td>

                                <td>
                                    <strong>${lopHoc.tenLop}</strong>
                                </td>

                                <td>${lopHoc.coVan}</td>

                                <td>${lopHoc.soLuongSinhVien}</td>

                                <td>

                                    <div class="actions">

                                        <a class="btn btn-primary"
                                           href="${pageContext.request.contextPath}/lophoc?action=detail&id=${lopHoc.id}">
                                            Xem
                                        </a>

                                        <a class="btn btn-warning"
                                           href="${pageContext.request.contextPath}/lophoc?action=form&id=${lopHoc.id}">
                                            Sửa
                                        </a>

                                        <a class="btn btn-danger"
                                           href="${pageContext.request.contextPath}/lophoc?action=delete&id=${lopHoc.id}"
                                           onclick="return confirm('Bạn có chắc muốn xóa lớp này không?')">
                                            Xóa
                                        </a>

                                    </div>

                                </td>

                            </tr>

                        </c:forEach>

                    </c:otherwise>

                </c:choose>

                </tbody>

            </table>

        </div>

    </div>

</div>

<div class="footer">
    LAB 07 - CRUD MVC | Java Web Servlet JSP
</div>

</body>
</html>